...
public class VitalHistory {
    ...
    private static final String TAG = "Glucose History";

    public VitalHistory(ReactContext reactContext, GoogleFitManager googleFitManager, DataType dataType){
        this.mReactContext = reactContext;
        this.googleFitManager = googleFitManager;
        this.dataType = dataType;
    }

    public VitalHistory(ReactContext reactContext, GoogleFitManager googleFitManager){
        this(reactContext, googleFitManager, HealthDataTypes.TYPE_BLOOD_GLUCOSE);
    }
    ...
    public ReadableArray getHistory(long startTime, long endTime) {
        Log.i(TAG, "GET GLUCOSE HISTORY ANDROID");
        ...
        if (this.dataType == HealthDataTypes.TYPE_BLOOD_GLUCOSE) {
        ...
        DataSet dataSet = DataSet.create(bloodGlucoseSource);
        DataPoint bloodGlucose = dataSet.createDataPoint().setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS);
        bloodGlucose.getValue(HealthFields.FIELD_BLOOD_GLUCOSE_LEVEL).setFloat(5.0f);
        bloodGlucose.getValue(HealthFields.FIELD_TEMPORAL_RELATION_TO_MEAL)
                .setInt(HealthFields.FIELD_TEMPORAL_RELATION_TO_MEAL_BEFORE_MEAL);
        bloodGlucose.getValue(Field.FIELD_MEAL_TYPE).setInt(Field.MEAL_TYPE_BREAKFAST);
        bloodGlucose.getValue(HealthFields.FIELD_BLOOD_GLUCOSE_SPECIMEN_SOURCE)
        ...
        if (!insertStatus.isSuccess()) {
            Log.i(TAG, "There was a problem inserting the dataset." + insertStatus);
        } else {
...
//        WritableMap stepMap = Arguments.createMap();
        ...
        for (DataPoint dp : dataSet.getDataPoints()) {
            ...
            for(Field field : dp.getDataType().getFields()) {
                ...
                if (this.dataType == HealthDataTypes.TYPE_BLOOD_GLUCOSE) {
                    stepMap.putDouble("value", dp.getValue(HealthFields.FIELD_BLOOD_GLUCOSE_LEVEL).asFloat());
                } else {
